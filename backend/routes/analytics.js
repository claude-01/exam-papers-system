...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        // Run all queries in parallel (only using columns that exist)
        const [
            [totalVisitsResult],
            [todayVisitsResult],
            [monthVisitsResult],
            ...
            db.query(
                `SELECT p.id, p.subject, p.year, p.level, COUNT(d.id) as download_count 
                 FROM exam_papers p 
        ...
        res.status(500).json({ 
            ...
            data: {
                ...
                papers: { total: 0, active: 0, olevel: 0, alevel: 0, byYear: [], bySubject: [] },
