...
async function loadPapers() {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE}/api/papers/public`);
...
async function filterByCategory(category) {
    ...
    if (category === 'all') {
        ...
        try {
            const response = await fetch(`${API_BASE}/api/public/papers/category/${category}`);
...
async function filterByLevel(level) {
    ...
    try {
        const response = await fetch(`${API_BASE}/api/public/papers/level/${level}`);
        const result = await response.json();
...
async function downloadPaper(id, fileUrl) {
    try {
        await fetch(`${API_BASE}/api/analytics/track-download`, {
            method: 'POST',
...
async function trackVisit() {
    try {
        await fetch(`${API_BASE}/api/analytics/track-visit`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
    } catch (error) {
